...
function CombatProtocol.AddToDisplayTab(guiName, ctrl, line)
  ...
  local function UpdateCombatTime()
    ...
    --if CombatProtocol.store.database then
    --  protocolEntries:SetText( #CombatProtocol.store.database )
    --end
  end
...
function CombatProtocol.OnEvent(event, ...)
  ...
  --if ExoY.combatProtocol.IsAbilityOnIgnolist( protocolEntry.data["abilityId"] ) then return end
  ...
  if protocolEntry.data.sourceType == "npc" or  protocolEntry.data.sourceType == "other" then
    ...
    if event == EVENT_COMBAT_EVENT then
      output = zo_strformat("[<<2>>]<<3>> from <<6>> to <<1>> (<<4>>) - <<5>>", protocolEntry.data.targetUnitId, protocolEntry.combatTime, protocolEntry.data.abilityId, protocolEntry.data.result, protocolEntry.data.hitValue, protocolEntry.data.sourceUnitId)
    else
      output = zo_strformat("[<<2>>]<<3>> on <<1>> ><<6>>< (<<4>>) - <<5>>", protocolEntry.data.unitId, protocolEntry.combatTime, protocolEntry.data.abilityId, protocolEntry.data.changeType, protocolEntry.data.hitValue, protocolEntry.data.unitName)
    end
    table.insert(CombatProtocol.store.database, output)
  end
